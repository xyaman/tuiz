...
pub fn draw(self: *Self, buf: *Buffer) void {
    ...
    if (self.text) |text| {
        ...
        const initial_col = self.box.rect.col + 1;
        const initial_row = self.box.rect.row + 1;

        var curr_row = initial_row;
        while (curr_row < initial_row + self.box.rect.h - 1) : (curr_row += 1) {
            var curr_col = initial_col;
            const row = (curr_row - initial_row) * (self.box.rect.w - 2);

            // TODO: change return. check index in while condition
            while (curr_col < initial_col + self.box.rect.w - 1) : (curr_col += 1) {
